<?php
ini_set('default_charset','UTF-8');
include_once("PHP/ValidaSearch.php");
?>
<!DOCTYPE HTML>
<html lang="PT-BR">
<head>
<meta charset="UTF-8">
<title>Página Inicial</title>
<meta name="viewport" content="initial-scale=1">
<link rel="stylesheet" type="text/css" href="css/index.css">
<link rel="stylesheet" type="text/css" href="css/menu.css">
<link rel="stylesheet" type="text/css" href="css/tabela.css">
<link rel="stylesheet" type="text/css" href="../css/Responsivo/MenuMobile.css">
<script src="JS/Tabela.js"></script>
</head>
<body>
<header>
<nav>
<img width="50" height="50" src="https://img.icons8.com/ios/50/menu--v1.png">
 <ul>
  <li><a href="Paginainicial.php" title="Página Inicial" class="PaginaInicialNav">Página Inicial</a></li>
  <li><a href="Produtos.php" title="Produtos">Produtos</a></li>
  <li><a href="estoque.php" title="Estoque">Estoque</a></li>
  <li><a href="CadastrarProduto.php" title="Cadastrar Produtos">Cadastrar Produtos</a></li>
 </ul>
</nav>
</header>
<section>
<h1 title="Produtos" class="Produtos">Produtos</h1>
  <form action="Search.php" method="get">
     <select name="SearchOption" id="SearchOption" >
      <option>---</option>
      <option>Masculino</option>
      <option>Feminino</option>
      <option>Infantil</option>
      <option>Todos</option>
    </select>
     <input type="image" src="data:image/png;base64," alt="Pesquisar" title="Pesquisar">
    </form>
</section>

<?php
 include_once("PHP/FunctionsDB/MaisVendidos.php");
 include_once("PHP/Banco.php");
 $BestSellingProduct = new BestSellingProduct();
$BestSellingProduct -> GetBestSellingProduct($conecta);
$BestSellingProduct -> BestSellingProduct();
?>

</body>
</html>
